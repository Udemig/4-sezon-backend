...
export const createReview = async (req, res, next) => {
  ...
  try {
    ...
    if (oldReview)
      return next(error(403, "Zaten bu hizmete yorumunuz var"));
...
export const deleteReview = async (req, res, next) => {
  try {
    ...
    // 2) yorum farklı kullanıcya ait ise işlemi durdur
    if (req.userId != review.user)
      return next(
        error(403, "Sadece kendi yorumlarınızı kaldırabilirsiniz")
      );
